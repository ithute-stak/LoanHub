...
export function CdasBookingFailuresView() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError("Could not load CDAS exceptions and issues.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function recordFailure() {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || "Could not record this booking issue.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function retryFailure(id: string) {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || "Could not reopen this booking issue.");
    } finally {
      setRetryingId(null);
    }
  }
  ...
  if (!data && loading) {
    return <Card><CardContent className="py-16 text-center text-sm text-muted-foreground">Loading booking issues…</CardContent></Card>;
  }
  if (!data) {
    return <Card><CardContent className="py-10 text-center text-sm text-destructive">{error || "Booking issues could not be loaded."}</CardContent></Card>;
  }
  ...
  return <div className="space-y-5">
    <Card>
      <CardHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <CardTitle>CDAS Exceptions & Issues</CardTitle>
              <CardDescription>Record why a booking could not proceed, preserve every issue attempt, and reopen only cases that are explicitly retryable.</CardDescription>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Issue attempts</div><div className="mt-1 text-xl font-semibold">{data.summary.failure_attempts}</div></div>
          <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Needs intervention</div><div className="mt-1 text-xl font-semibold">{data.summary.currently_failed}</div></div>
          ...
          <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">Manual review only</div><div className="mt-1 text-xl font-semibold">{data.summary.non_retryable}</div></div>
        </div>
    ...
    <Card>
      <CardHeader><CardTitle>Record a booking issue</CardTitle><CardDescription>Select an active opportunity. Recording the issue moves it out of the active booking flow until it is reviewed or explicitly reopened.</CardDescription></CardHeader>
      <CardContent className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="lg:col-span-2"><Button onClick={() => void recordFailure()} disabled={saving || !opportunityId || !reasonCode}>{saving ? "Recording…" : "Record issue"}</Button></div>
      </CardContent>
    </Card>

    <Card>
      <CardHeader><CardTitle>Issue history</CardTitle><CardDescription>Previous booking issues remain visible even after an opportunity is reopened and later succeeds.</CardDescription></CardHeader>
      <CardContent className="space-y-4">
        <input aria-label="Search booking issues" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search client, agency, reference or issue reason" className="h-10 w-full rounded-md border bg-background px-3 text-sm sm:max-w-lg"/>
        {!historyItems.length ? <div className="py-10 text-center text-sm text-muted-foreground">No booking issues have been recorded yet.</div> : <div className="grid gap-4 lg:grid-cols-2">{historyItems.map((item) => {
          ...
          return <Card key={item.id} className={failedNow ? "border-amber-500/40" : ""}>
            <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{item.client_name || item.client_reference || "CDAS client"}</CardTitle><CardDescription>{item.opportunity_agency_name || "Agency not captured"} • {money(item.opportunity_deduction_amount)}</CardDescription></div><Badge variant={failedNow ? "outline" : "secondary"}>{failedNow ? "Needs intervention" : item.pipeline_stage.replaceAll("_", " ")}</Badge></div></CardHeader>
            <CardContent className="space-y-3 text-sm">
              {latest && <div className="rounded-lg border p-3"><div className="font-medium">{latest.reason_label}</div><div className="mt-1 text-xs text-muted-foreground">Recorded {dateTime(latest.failed_at)}</div>{latest.reason_details && <p className="mt-2 text-sm">{latest.reason_details}</p>}<div className="mt-2 text-xs text-muted-foreground">{latest.retry_eligible ? `Retry ${latest.retry_after ? `after ${dateTime(latest.retry_after)}` : "allowed immediately"}` : "Manual review only"}</div></div>}
              <div className="flex items-center justify-between gap-3"><span className="text-xs text-muted-foreground">{item.failure_count} recorded issue attempt{item.failure_count === 1 ? "" : "s"}</span>{failedNow && latest?.retry_eligible && <Button size="sm" variant={item.retry_due ? "default" : "outline"} disabled={!item.retry_due || retryingId === item.id} onClick={() => void retryFailure(item.id)}><RotateCcw className="mr-2 h-4 w-4"/>{item.retry_due ? (retryingId === item.id ? "Reopening…" : "Reopen for retry") : "Retry not due"}</Button>}</div>
              {item.failure_history.length > 1 && <details><summary className="cursor-pointer text-xs font-medium">View earlier issues</summary><div className="mt-2 space-y-2">{item.failure_history.slice(1).map((failure) => <div key={failure.id} className="rounded border p-2 text-xs"><div className="font-medium">{failure.reason_label}</div><div className="text-muted-foreground">{dateTime(failure.failed_at)}</div></div>)}</div></details>}
            </CardContent>
          </Card>;
        })}</div>}
      </CardContent>
