...
import { cdasDashboardApi, type CdasDashboardKpis, type CdasMovement } from "@/api/cdasDashboard";
...
export default function CompanyDashboardPage() {
    ...
    const historyMax = useMemo(
        () => Math.max(1, ...(cdasKpis?.run_rate_history.map((item) => item.amount) ?? [0])),
        [cdasKpis?.run_rate_history],
    );
    ...
    return (
        <div className="space-y-4 2xl:space-y-6">
            ...
            <section className="overflow-hidden rounded-2xl border bg-card p-4 shadow-sm sm:p-5 2xl:rounded-3xl 2xl:p-8">
            ...
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 2xl:gap-4">
            ...
            <section className="rounded-2xl border bg-card p-4 shadow-sm 2xl:rounded-3xl 2xl:p-5">
                <div className="flex flex-col gap-2 border-b pb-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-xs leading-5 text-muted-foreground 2xl:text-sm">
                            Company-linked payroll deductions, forward schedules and exact-ID collection opportunities.
                        </p>
                    </div>
                ...
                {cdasLoading && !cdasKpis ? (
                    ...
                    <div className="mt-4 space-y-4">
                        ...
                        <div className="grid gap-3 xl:grid-cols-[1.2fr_0.8fr]">
                            <div className="rounded-xl border bg-muted/20 p-3 2xl:p-4">
                                <div className="flex items-center justify-between gap-3">
                                    <div>
                                        <p className="text-sm font-black">6-month contracted run-rate</p>
                                        <p className="mt-0.5 text-xs text-muted-foreground">Lightweight view based on LoanHub-linked CDAS mandate lifecycle dates.</p>
                                    </div>
                                    <span className="text-xs font-bold text-muted-foreground">LSL / month</span>
                                </div>
                                <div className="mt-4 grid h-24 grid-cols-6 items-end gap-2 sm:h-28">
                                    {cdasKpis.run_rate_history.map((item) => {
                                        const height = item.amount <= 0 ? 4 : Math.max(10, Math.round((item.amount / historyMax) * 100));
                                        return (
                                            <div key={item.month} className="flex h-full min-w-0 flex-col justify-end gap-1">
                                                <div className="truncate text-center text-[10px] font-bold text-muted-foreground">{item.amount > 0 ? formatMoney(item.amount) : "M0"}</div>
                                                <div className="flex h-16 items-end rounded-md bg-muted/50 sm:h-20">
                                                    <div className="w-full rounded-md bg-primary/75" style={{ height: `${height}%` }} />
                                                </div>
                                                <div className="truncate text-center text-[10px] text-muted-foreground">{monthLabel(item.month)}</div>
                                            </div>
                                        );
                                    })}
                                </div>
