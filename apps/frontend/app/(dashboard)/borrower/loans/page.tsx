    ...
    <div className="loanhub-page">
      <section className="loanhub-hero flex flex-col justify-between gap-5 p-6 lg:flex-row lg:items-end">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Every loan has a permanent folio number that stays with that loan for its full history. Use the folio shown here when referring to your account; LoanHub also keeps the technical loan reference for reconciliation and integrations.</p>
        </div>
      ...
      <Alert>
        ...
        <AlertTitle>Your permanent loan identity</AlertTitle>
        <AlertDescription>Use your folio number when contacting the lender or checking a payment. Your receipts and loan documents also carry the same folio so the transaction can be traced back to the correct sequence-book entry.</AlertDescription>
      </Alert>

      <div className="grid gap-5 lg:grid-cols-2">
        {loans.length === 0 ? <Card className="loanhub-panel lg:col-span-2"><CardContent className="flex min-h-72 flex-col items-center justify-center p-8 text-center"><HandCoins className="h-12 w-12 text-muted-foreground" /><h2 className="mt-4 text-xl font-black">No loans yet</h2><p className="mt-2 max-w-xl text-sm text-muted-foreground">When you accept a loan offer or a company approves an internal application, the loan and its permanent folio number will appear here.</p></CardContent></Card> : loans.map((loan) => {
          ...
          return <Card key={loan.id} className="loanhub-panel overflow-hidden">
            <CardHeader className="border-b bg-gradient-to-br from-primary/8 to-emerald-500/8">
              <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-sm font-black text-primary">{loan.folio_number}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">Loan {loan.loan_reference}</p><CardTitle className="mt-2">{formatMoney(loan.principal_amount)} loan</CardTitle><CardDescription>{loan.folio_group_code} sequence {loan.folio_sequence} · {interestMethodLabel(loan.calculation_method)} · {loan.repayment_period} months</CardDescription></div><Badge variant={loan.status === "active" || loan.status === "completed" ? "default" : "secondary"}>{titleCase(loan.status)}</Badge></div>
            </CardHeader>
