...
def attention_notification_filter():
    return or_(func.lower(Notification.priority).in_(ATTENTION_PRIORITIES), Notification.notification_type.in_(ATTENTION_NOTIFICATION_TYPES))
...
def list_notifications(
    ...
    total = query.count()
    unread_query = db.query(func.count(Notification.id)).filter(Notification.user_id == current_user.id, Notification.is_read.is_(False), Notification.is_archived.is_(False))
    if (attention_only or not include_routine) and not event_type:
    ...
    return NotificationListRead(items=[NotificationRead.model_validate(item) for item in items], total=total, unread_count=unread_count, page=page, page_size=page_size)
...
def mark_notification_read(notification_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def mark_all_read(include_routine: bool = False, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    query = db.query(Notification).filter(Notification.user_id == current_user.id, Notification.is_read.is_(False), Notification.is_archived.is_(False))
    if not include_routine:
        query = query.filter(attention_notification_filter())
    query.update({Notification.is_read: True, Notification.read_at: datetime.utcnow()}, synchronize_session=False)
    db.commit()
    return Response(status_code=status.HTTP_204_NO_CONTENT)


@router.patch("/{notification_id}/archive", response_model=NotificationRead)
def archive_notification(notification_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    notification = notification_or_404(db, notification_id, current_user.id)
