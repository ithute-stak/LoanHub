...
router = APIRouter(prefix="/cdas-booking", tags=["CDAS Data Quality"])
...
def get_cdas_data_quality_centre(
    context: TenantContext = Depends(get_tenant_context),
    db: Session = Depends(get_db),
):
    """Return data-quality diagnostics for each exact client's latest archived CDAS analysis."""
    ...
    return build_data_quality_centre(profiles)
